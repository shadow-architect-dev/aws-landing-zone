...
7. **高度な SRE ＆ 可観測性自律化（Drift / Anomaly / Chaos / ChatOps）**:
   - **Drift Detection (設定の逸脱検出)**: セキュリティグループの危険なポートの無制限開放などを AWS Config ＆ EventBridge で常時監視・自動検知。
...
10. **組織中央セキュリティガードレール (Policy-as-Code)**:
    - **OIDC ブランチ制限 (最小権限の強制)**: 本番環境（`prod`）の GitHub Actions デプロイロール（IAM）は、`main` ブランチからのリクエストのみ AssumeRole（ロール引き受け）できるよう厳格化。誤って開発ブランチから本番へデプロイされる事故を物理遮断。
    - **コンテナ非Root実行監査 (AWS Config)**: 組織内の全アカウントを対象に、ECS タスク定義などでコンテナが Root（管理者）権限で起動される設定になっていないかを自動的に発見監視。
    - **DB/Cache 送信制限監査 (AWS Config)**: データベースやキャッシュのセキュリティグループに、送信先を制限しない全送信（0.0.0.0/0）許可ルールが存在しないかを組織全体で常時監視し、データ漏洩経路を排除。
11. **アカウント隔離用 Suspended OU ＆ 隔離用 SCP (ライフサイクル管理ベストプラクティス)**:
    - **安全なアカウントオフボーディング**: 不要になったワークロードアカウントを即座に削除せず、一度 `Suspended` 組織単位（OU）へ GitOps 移動して隔離。
    - **DenyAll SCP による検疫**: `Suspended` OU にアタッチされた強力な `DenyAll` ポリシーにより、対象アカウント内のすべての API 実行と外部通信を AWS レベルで完全ロック。一定期間の「冷却期間（悲鳴テスト期間）」を経て安全が確認された後に完全クローズすることで、予期せぬ重要データの完全消失を未然に防止。
...
*   `policies/` - 組織共通ガードレール（SCP）およびタグポリシー定義（JSON）
    *   `scp/`: 許可リージョン制限（東京のみ）、監査機能無効化防止、本番データ削除防止（S3バージョニング保護含む）、およびアカウント隔離用 `deny-all`（全アクション拒否）の SCP 定義。
...
*   **自律的ドリフト検知 (Drift Detection)**: セキュリティグループの意図しないポート無制限開放などを AWS Config ＆ EventBridge でリアルタイム検知し、即座に Slack へ ChatOps 通知。
...
*   **機械学習コスト異常検出 (Cost Anomaly Detection)**: コストの急激な変化（日次しきい値超え）を検知し、Slack 連携で FinOps 統制を自動可視化。
...
*   **カオスエンジニアリング基盤による SLO 検証**: AWS FIS (Fault Injection Service) をコード定義。擬似障害（Bastion/NAT の一時停止等）を安全に注入し、SLO に定めた可用性の回復力や監視遅延を定期検証。
...
*   **統合通知と ChatOps**: AWS Chatbot と Slack アラートを統合し、アカウント作成完了通知や脅威アラート（GuardDuty/SecurityHub）を一箇所に集約。
